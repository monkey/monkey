...
static int config_parse(const char *confdir, struct polar_config *conf)
{
    ...
    cert_file = mk_api->config_section_getval(section,
                                              "CertificateFile",
                                              MK_CONFIG_VAL_STR);
    cert_chain_file = mk_api->config_section_getval(section,
                                                    "CertificateChainFile",
                                                    MK_CONFIG_VAL_STR);
    key_file = mk_api->config_section_getval(section,
                                             "RSAKeyFile",
                                             MK_CONFIG_VAL_STR);
    dh_param_file = mk_api->config_section_getval(section,
                                                  "DHParameterFile",
                                                  MK_CONFIG_VAL_STR);
fallback:
...
int _mkp_init(struct plugin_api **api, char *confdir)
{
    ...
    if (mk_api->config->transport_layer &&
        strcmp(mk_api->config->transport_layer, "tls")) {
        PLUGIN_TRACE("[tls] Not used as transport layer, unload.");
        return -1;
    }
