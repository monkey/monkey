 ...
 *  Copyright (C) 2011, Jonathan Gonzalez V.
 *
...
MONKEY_PLUGIN("liana_ipv6",         /* shortname */
              "Liana Network", /* name */
              VERSION,        /* version */
...
int _mkp_network_io_bind(int socket_fd, const struct sockaddr_in6 *addr, socklen_t addrlen, int backlog)
{
    int ret;
...
int _mkp_network_io_server(int port, char *listen_addr)
{
    int socket_fd;
    int ret;
    struct sockaddr_in6 local_sockaddr_in;

    socket_fd = _mkp_network_io_create_socket(PF_INET6, SOCK_STREAM, 0);
    if( socket_fd == -1) {
    ...
    ret = _mkp_network_io_bind(socket_fd, (struct sockaddr_in6 *) &local_sockaddr_in,
                               sizeof(struct sockaddr_in6), MK_SOMAXCONN);
